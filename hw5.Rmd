...
```{r}
city_sum %>% 
  mutate(city_state <- factor(city_sum$city_state, levels = city_sum$city_state[order(city_sum$estimate)])) %>% 
  ...
  theme(
    legend.key = element_rect(colour = 'purple', fill = 'pink', size = 0.03, linetype = 'dashed'),
